...
-- Clean up any existing rooms for these specific test hotels to avoid duplicates when re-seeding
DELETE FROM hotel_rooms WHERE hotel_id IN (
  SELECT id FROM hotels WHERE name IN ('Grand Plaza Hotel', 'Seaside Resort', 'Mountain Lodge')
);
