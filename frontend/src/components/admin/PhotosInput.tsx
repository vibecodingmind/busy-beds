...
export default function PhotosInput({ value, onChange, placeholder = 'https://example.com/photo.jpg', maxPhotos = 20 }: PhotosInputProps) {
  ...
  return (
    <div>
      <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Photos</label>
      ...
      <div className="mt-2 space-y-2">
        ...
        {value.length < maxPhotos && (
          <button
            type="button"
            onClick={addPhoto}
            className="rounded-lg border border-dashed border-zinc-300 px-4 py-2 text-sm text-zinc-600 hover:border-zinc-400 hover:bg-zinc-50 dark:border-zinc-600 dark:text-zinc-400 dark:hover:bg-zinc-800"
          >
            + Add photo
          </button>
        )}
      ...
      {value.length === 0 && (
        <button
          type="button"
          onClick={addPhoto}
          className="mt-2 rounded-lg border border-zinc-300 bg-zinc-50 px-4 py-2 text-sm text-zinc-700 hover:bg-zinc-100 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700"
        >
          Add first photo
        </button>
      )}
