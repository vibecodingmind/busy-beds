...
function HotelsClientInner() {
  ...
  const [viewMode, setViewMode] = useState<'split' | 'list' | 'map'>(nearme ? 'split' : 'split');
  ...
  const fetchHotels = useCallback(async (reset = false) => {
    ...
    try {
      ...
      if (reset && result.hotels.length > 0) {
        const uniqueCities = [...new Set(result.hotels.map((h) => h.location).filter(Boolean))] as string[];
        setCities(uniqueCities.slice(0, 20));
      }
    } catch (err) {
  ...
  useEffect(() => {
    fetchHotels(true);
  }, [debouncedSearch, sort, coords, debouncedBounds, filters.minPrice, filters.maxPrice, filters.minRating, filters.hasDiscount, filters.amenities]);
    ...
    <div className="min-h-screen">
            ...
            <div className="hidden md:flex rounded-xl border border-black/10 dark:border-zinc-700 overflow-hidden">
              <button
                ...
                className={`px-4 py-2 text-sm transition-colors flex items-center gap-2 ${
                  viewMode === 'list' ? 'bg-zinc-200 dark:bg-zinc-700 text-black dark:text-zinc-100' : 'hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-300'
                }`}
              >
              ...
              <button
                ...
                className={`px-4 py-2 text-sm transition-colors flex items-center gap-2 ${
                  viewMode === 'split' ? 'bg-zinc-200 dark:bg-zinc-700 text-black dark:text-zinc-100' : 'hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-300'
                }`}
              >
                Split
              </button>
              <button
                ...
                className={`px-4 py-2 text-sm transition-colors flex items-center gap-2 ${
                  viewMode === 'map' ? 'bg-zinc-200 dark:bg-zinc-700 text-black dark:text-zinc-100' : 'hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-300'
                }`}
              >
            ...
            {hasMore && (
              <div className="mt-6 text-center">
                <button
                  ...
                  className="px-6 py-2.5 rounded-xl bg-primary text-white font-medium hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      {!loading && displayedHotels.length === 0 && (
        <div className="py-12 text-center text-zinc-500 dark:text-zinc-400">
          ...
          <p className="mt-1 text-sm">Try adjusting your filters or map area</p>
        </div>
      )}
